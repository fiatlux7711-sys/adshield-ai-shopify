...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  try {
    ...
    // A 3xx is the library's normal way of driving the OAuth flow (e.g.
    // sending the merchant to Shopify's consent screen) — not a failure.
    // A 401/403 is a real authentication failure.
    if (error instanceof Response) {
      if (error.status >= 400) {
        logger.error("auth.request_failed", {
          path: url.pathname,
          shop,
          status: error.status,
